...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 09:11:59 2021
...
#### Load EEG data
#eeg = "/scratch/dcas/g.singh/features/lh_EEG_Signal.pickle"
            ...
            for p in range(participants):
                for e in range(0, epochs, 1):         
                    ...
                    if p == i:    #### Data for testing                        
                        for c in range(32): #channels only first 32 which are eeg channels
                            ltr_filt = butter_bandpass_filter(lh_eeg[p,l,0,e,c,:], 4, 45, 512, order=6)                        
                            ltr_filt = np.expand_dims(np.expand_dims(ltr_filt, axis=0), axis=0)
                            
                            lte_filt = butter_bandpass_filter(lh_eeg[p,l,1,e,c,:], 4, 45, 512, order=6)
                            lte_filt = np.expand_dims(np.expand_dims(lte_filt, axis=0), axis=0)
...
pickle_out = open("./output/log_eeg_eco_csp_4bands.pickle", "wb")
